...
export default function DemoDataPage() {
  ...
  const run = (action: () => boolean, success: string) => setMessage(action() ? success : "Kh?ng th? c?p nh?t d? li?u tr?n tr?nh duy?t n?y.");
  return <div className="min-h-screen md:flex"><Sidebar /><main className="flex-1 p-4 sm:p-5 md:p-8">
    ...
    <section className="card max-w-3xl space-y-4 p-5">
      <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">D? li?u m?u ch? ph?c v? ki?m tra tr?n tr?nh duy?t hi?n t?i.</div>
