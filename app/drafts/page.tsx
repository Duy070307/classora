...
import { draftToolNames, legacyDraftToolKeys } from "@/lib/draft-tool-names";
...
export default function DraftsPage() {
  ...
  const filtered = drafts.filter((draft) =>
    `${draftToolNames[draft.toolKey] || draft.toolKey} ${preview(draft.data)}`
      .toLowerCase()
    ...
    <div className="min-h-screen md:flex">
      ...
      <main className="flex-1 p-5 md:p-8">
        <PageHeader
          ...
          description="Các thông tin đang nhập được Soạn Lab tự lưu tạm trên trình duyệt này."
        />
        {drafts.length ? (
          <>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {filtered.map((draft) => {
                const isLegacy = legacyDraftToolKeys.has(draft.toolKey);
                return (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <h2 className="font-bold text-ink">
                          {isLegacy
                            ? "Nội dung cũ"
                            : draftToolNames[draft.toolKey] || draft.toolKey}
                        </h2>
          ...
          <SoanLabEmptyState
            ...
            description="Khi bạn nhập thông tin trong các công cụ, Soạn Lab sẽ tự lưu tạm trên trình duyệt."
          />
        )}
      </main>
    </div>
