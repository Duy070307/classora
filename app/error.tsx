...
export default function ErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("Classora route error", error);
  }, [error]);
  ...
  return <main className="flex min-h-screen items-center justify-center px-4 py-12">
    <section className="card w-full max-w-lg p-6 text-center sm:p-8">
      ...
      <p className="mt-3 leading-7 text-muted">Classora chưa thể hiển thị nội dung này. Bạn có thể thử lại hoặc quay về dashboard.</p>
