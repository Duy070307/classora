...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { draft?: unknown; format?: string; fileName?: string; settings?: { background?: "transparent" | "white"; lineWeight?: "thin" | "standard" | "bold"; showLabels?: boolean; scale?: 1 | 2 | 4 } }; const format = body.format || "svg";
    ...
    const assets = await buildTikzAssets(draft, body.settings); const data = format === "png" ? assets.png : assets.svg; const contentType = format === "png" ? "image/png" : "image/svg+xml; charset=utf-8";
