...
const shareMessage = "Em muốn giới thiệu Soạn Lab, một bộ công cụ hỗ trợ giáo viên tạo đề, ma trận đề, phiếu học tập, nhận xét học sinh và xuất Word/PDF. Cô/thầy có thể mở mẫu sử dụng, tạo một bản nháp tài liệu, xuất Word/PDF rồi rà soát và chỉnh sửa trước khi dùng.";
...
export default function SharePage() {
  ...
  return <main className="warm-page min-h-screen"><Navbar /><section className="mx-auto max-w-4xl px-4 py-10 sm:py-16">
    <div className="rounded-3xl bg-gradient-to-br from-blue-600 to-indigo-700 p-6 text-white shadow-xl shadow-blue-200 sm:p-9">
      ...
      <p className="mt-4 max-w-2xl leading-7 text-blue-100">Mời đồng nghiệp khám phá các công cụ hỗ trợ soạn tài liệu dành cho giáo viên.</p>
    </div>
    <section className="play-card mt-8 overflow-hidden">
      <div className="border-b border-blue-100 bg-blue-50/60 px-5 py-4 text-sm font-extrabold text-blue-900">Tin nhắn giới thiệu Soạn Lab</div>
    ...
    <section className="play-card mt-8 p-5 sm:p-6">
      ...
      <ol className="mt-4 grid gap-3 sm:grid-cols-2">
        {flow.map((item, index) => <li key={item} className="flex gap-3 rounded-2xl bg-slate-50 p-4 text-sm font-semibold text-slate-700"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-xs font-black text-white">{index + 1}</span>{item}</li>)}
      </ol>
    </section>
