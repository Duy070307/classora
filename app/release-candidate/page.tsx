...
export default function ReleaseCandidatePage() {
  ...
  return <main className="warm-page min-h-screen"><Navbar /><section className="mx-auto max-w-6xl px-4 py-10 sm:py-14">
    <div className="hero-gradient relative overflow-hidden rounded-[30px] p-6 text-white shadow-[0_20px_50px_rgba(37,99,235,.2)] sm:p-9"><div className="absolute -right-16 -top-20 h-64 w-64 rounded-full border-[36px] border-white/10" /><div className="relative"><p className="text-xs font-extrabold uppercase tracking-[.16em] text-cyan-200">Kiểm tra chất lượng</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">Trạng thái sản phẩm</h1><p className="mt-4 max-w-3xl leading-7 text-blue-100">Bảng kiểm tra các quy trình chính của Soạn Lab trước khi giáo viên sử dụng.</p><div className="mt-6 flex flex-wrap gap-2">{[[CheckCircle2,"Sẵn sàng"],[FlaskConical,"Đang kiểm tra"],[HardDrive,"Lưu trên trình duyệt"],[Sparkles,"Tạo bản nháp"]].map(([Icon,label])=>{const I=Icon as typeof CheckCircle2;return <span key={label as string} className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-2 text-xs font-bold ring-1 ring-white/20"><I size={14}/>{label as string}</span>;})}</div></div></div>
    ...
    <div className="mt-7 flex flex-wrap gap-3"><Link href="/dashboard" className="btn-primary">Mở dashboard</Link><Link href="/tools" className="btn-secondary">Mở tất cả công cụ</Link><Link href="/demo-checklist" className="btn-secondary">Kiểm tra chi tiết</Link><Link href="/known-issues" className="btn-secondary">Trạng thái hệ thống</Link><Link href="/feedback" className="btn-secondary">Gửi góp ý</Link></div>
