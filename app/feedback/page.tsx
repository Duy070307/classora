...
${browserNote}`;
  ...
  return <main className="warm-page min-h-screen"><Navbar /><section className="mx-auto max-w-5xl px-4 py-10 md:py-14"><div className="hero-gradient mb-8 rounded-[30px] p-6 text-white shadow-[0_18px_44px_rgba(37,99,235,.18)] sm:p-8"><p className="text-xs font-extrabold uppercase tracking-[.16em] text-cyan-200">Góp ý sản phẩm</p><h1 className="mt-3 text-3xl font-black md:text-5xl">Góp ý cho Soạn Lab</h1><p className="mt-4 max-w-3xl leading-7 text-blue-100">Thầy cô hãy giúp đánh giá độ dễ dùng, quy trình và chất lượng file Word/PDF để Soạn Lab ngày càng hữu ích hơn.</p></div>
    <div className="grid gap-6 lg:grid-cols-[0.75fr_1.25fr]"><aside className="play-card h-fit p-5 sm:p-6"><h2 className="text-lg font-extrabold text-ink">Câu hỏi gợi ý góp ý</h2><ol className="mt-4 space-y-2 text-sm leading-6 text-muted"><li>1. Công cụ nào hữu ích nhất?</li><li>2. File Word/PDF có dùng được không?</li><li>3. Form nhập liệu có dễ hiểu không?</li><li>4. Nội dung tạo ra cần cải thiện gì?</li><li>5. Thầy/cô muốn thêm công cụ nào?</li></ol><div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6 text-blue-900"><div className="flex gap-2"><MessageCircle size={16} className="mt-0.5 shrink-0" /><p>Thông tin chỉ được sao chép, không gửi lên server.</p></div></div></aside>
