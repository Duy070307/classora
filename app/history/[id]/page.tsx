...
import { blueprintToDocument, normalizeWorkflowBlueprint } from "@/lib/exam-blueprint/workflow";
...
import { downloadWorksheetDocx, printWorksheetPdf } from "@/lib/worksheet/export";
import { openLessonPlanGenerator } from "@/lib/lesson-plan/session";
import { downloadLessonPlanDocx, printLessonPlanPdf } from "@/lib/lesson-plan/export";
import { lessonPlanToDocument } from "@/lib/lesson-plan/workflow";
...
export default function HistoryDetailPage() {
  ...
  return <div className="min-h-screen md:flex"><Sidebar /><main className="flex-1 p-5 md:p-8">
    ...
    {document.examBlueprint ? <div className="mb-4 flex flex-wrap gap-2"><button type="button" className="btn-primary" onClick={()=>openExamBlueprint({mode:"edit",blueprintDocument:document})}><Eye size={16}/>Mở &amp; chỉnh sửa</button><button type="button" className="btn-secondary" onClick={()=>void downloadBlueprintXlsx(document.examBlueprint!)}><Download size={16}/>Excel</button><button type="button" className="btn-secondary" onClick={()=>void downloadBlueprintDocx(document.examBlueprint!)}><Download size={16}/>Word</button><button type="button" className="btn-secondary" onClick={()=>{const copy=normalizeWorkflowBlueprint({...document.examBlueprint!,id:crypto.randomUUID(),title:`${document.examBlueprint!.title||document.title} (bản sao)`});const next=blueprintToDocument(copy);saveDocument(next);router.push(`/history/${next.id}`);}}><Copy size={16}/>Nhân bản</button><button type="button" className="btn-secondary" onClick={()=>{sessionStorage.setItem(EXAM_BLUEPRINT_SESSION_KEY,JSON.stringify(document.examBlueprint));router.push('/tools/exam-generator?mode=file');}}><RefreshCw size={16}/>Tạo đề</button><button type="button" className="btn-secondary" onClick={()=>openExamBlueprint({mode:"compare",blueprintDocument:document})}><ClipboardCheck size={16}/>So sánh với đề</button></div>:null}
    {document.worksheet?<div className="mb-4 flex flex-wrap gap-2"><button className="btn-primary" onClick={()=>openWorksheetGenerator(document)}><Eye size={16}/>Mở &amp; chỉnh sửa</button><button className="btn-secondary" onClick={()=>void downloadWorksheetDocx(document.worksheet!,"student")}><Download size={16}/>Word học sinh</button><button className="btn-secondary" onClick={()=>void downloadWorksheetDocx(document.worksheet!,"teacher")}><Download size={16}/>Word giáo viên</button><button className="btn-secondary" onClick={()=>printWorksheetPdf(document.worksheet!,"student")}><Download size={16}/>PDF học sinh</button></div>:null}
    {document.lessonPlan?<div className="mb-4 flex flex-wrap gap-2"><button className="btn-primary" onClick={()=>openLessonPlanGenerator(document)}><Eye size={16}/>Mở &amp; chỉnh sửa</button><button className="btn-secondary" onClick={()=>void downloadLessonPlanDocx(document.lessonPlan!,"full")}><Download size={16}/>Word đầy đủ</button><button className="btn-secondary" onClick={()=>void downloadLessonPlanDocx(document.lessonPlan!,"teacher_student_table")}><Download size={16}/>Word bảng GV/HS</button><button className="btn-secondary" onClick={()=>printLessonPlanPdf(document.lessonPlan!,"full")}><Download size={16}/>PDF</button><button className="btn-secondary" onClick={()=>{const id=crypto.randomUUID();const copy=lessonPlanToDocument({...document.lessonPlan!,id,title:`${document.lessonPlan!.title} (bản sao)`,metadata:{...document.lessonPlan!.metadata,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}},"full");saveDocument(copy);router.push(`/tools/lesson-plan?history=${id}`);}}><Copy size={16}/>Nhân bản</button></div>:document.type==="lesson-plan"?<div className="mb-4"><button className="btn-primary" onClick={()=>openLessonPlanGenerator(document)}><Eye size={16}/>Chuyển sang trình chỉnh sửa</button></div>:null}
    ...
    {((!document.worksheet&&["lesson-plan","lesson-slides","exam","document-recognition"].includes(document.type))||document.examSolutionSet)?<div className="mb-4"><button className="btn-secondary" onClick={()=>openWorksheetGenerator(document,document.examSolutionSet?"solution":document.type==="exam"?"review":"practice")}><FileCheck2 size={16}/>Tạo phiếu học tập từ nội dung này</button></div>:null}
    {!document.lessonPlan&&["lesson-slides","worksheet","exam","rubric","document-recognition"].includes(document.type)?<div className="mb-4"><button className="btn-secondary" onClick={()=>openLessonPlanGenerator(document,document.type==="exam"?"solution":"new_lesson")}><FileCheck2 size={16}/>Tạo giáo án từ nội dung này</button></div>:null}
    <div className="mb-4 flex flex-wrap items-center gap-2"><DocumentExportMenu document={document} />{document.type === "document-recognition" ? <Link href={`/tools/document-recognition?history=${encodeURIComponent(document.id)}`} className="btn-primary"><FileCheck2 size={16} />Tiếp tục rà soát</Link> : null}{document.type === "exam" ? <Link href={`/tools/exam-audit?history=${encodeURIComponent(document.id)}`} className="btn-secondary"><ClipboardCheck size={16} />Kiểm tra lại</Link> : null}{document.structuredExam ? <button type="button" className="btn-secondary" onClick={() => openAnswerSolutions(document)}><FileCheck2 size={16} />Lời giải &amp; đáp án</button> : null}{document.structuredExam || document.examVariantSet ? <button type="button" className="btn-secondary" onClick={()=>openExamBlueprint({mode:"from_exam",document})}><RefreshCw size={16}/>Tạo ma trận từ đề này</button>:null}{document.type === "exam" ? <button type="button" className="btn-secondary" onClick={() => openExamMixer(document)}><Shuffle size={16} />Trộn mã đề</button> : null}{document.examVariantSet ? <Link href={`/tools/exam-mixer?history=${encodeURIComponent(document.id)}`} className="btn-secondary"><Shuffle size={16} />Mở bộ mã đề</Link> : null}{document.type === "exam" && document.generationMeta?.normalizedBlueprint ? <button type="button" className="btn-secondary" onClick={() => { sessionStorage.setItem(EXAM_BLUEPRINT_SESSION_KEY, JSON.stringify(document.generationMeta?.normalizedBlueprint)); router.push("/tools/exam-generator?mode=file"); }}><RefreshCw size={16} />Tạo đề mới theo cấu trúc này</button> : null}<button className="btn-secondary text-red-600" onClick={() => { if (window.confirm("Xóa tài liệu này khỏi lịch sử?")) { deleteDocument(document.id); void deleteCloudDocument(document.id); router.push("/history"); } }}><Trash2 size={16} />Xóa</button></div>
