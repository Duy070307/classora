...
export default function PricingPage() {
  return <AppShell title="Bảng giá demo">
    <section className="mb-7 overflow-hidden rounded-3xl bg-gradient-to-br from-blue-600 to-indigo-700 p-6 text-white shadow-xl shadow-blue-200 sm:p-9"><span className="rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-bold">MVP/demo</span><h1 className="mt-5 text-3xl font-extrabold sm:text-4xl">Bảng giá dự kiến</h1><p className="mt-3 max-w-2xl leading-7 text-blue-100">Các mức giá chỉ dùng để kiểm thử mô hình sản phẩm. Soạn Lab hiện chưa thu phí và không có thanh toán thật.</p></section>
    ...
    <p className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-medium leading-6 text-amber-800">Bảng giá là bản nháp để thu thập phản hồi. Không có giao dịch hoặc yêu cầu thanh toán trong bản demo.</p>
  </AppShell>;
}
