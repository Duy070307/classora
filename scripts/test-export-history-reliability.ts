...
fullExam.structuredExam = {
  ...
  parts: [
    ...
    { type: "true_false", title: "PHẦN II", instruction: "Chọn đúng hoặc sai.", questions: Array.from({ length: 4 }, (_, index) => ({ id: `tf-${index}`, part: "true_false" as const, number: index + 1, stem: `TF-${index + 1}-MARK: Xét các khẳng định về tính đơn điệu trường hợp ${index + 1}.`, trueFalseItems: ["a", "b", "c", "d"].map((label, item) => ({ label: label as "a", text: `Mệnh đề ${label} của trường hợp ${index + 1}`, answer: item % 2 === 0 })), answer: "a Đúng; b Sai; c Đúng; d Sai", explanation: `Lời giải TF ${index + 1}.`, score: 1, difficulty: "Thông hiểu" as const, topic: "hàm số" })) },
    { type: "short_answer", title: "PHẦN III", instruction: "Trả lời ngắn.", questions: Array.from({ length: 6 }, (_, index) => ({ id: `sa-${index}`, part: "short_answer" as const, number: index + 1, stem: `SA-${index + 1}-MARK: Tính giá trị f(${index + 1}).`, answer: `${index + 1}`, explanation: `Thay số và thu được kết quả ${index + 1}.`, score: 0.5, difficulty: "Vận dụng" as const, topic: "hàm số" })) },
  ],
