import type { ExamInput, GenericToolInput, WorksheetInput } from "@/lib/types";
...
export function mergeDefined<T extends Record<string, unknown>>(
  ...
  return Object.fromEntries(
    Object.entries({ ...base, ...values }).map(([key, value]) => [
      key,
      value ?? base[key],
    ]),
  ) as T;
...
export function getExamSamplePrefill(
  sampleId: string,
): Partial<ExamInput> | null {
  if (sampleId === "math-12-thptqg") {
    return {
      ...
      extraRequirements:
        "Tạo đề theo cấu trúc ba phần, nội dung là bản nháp cần giáo viên rà soát.",
    };
  }
  if (sampleId === "history-12-thptqg") {
    return {
      ...
      includeSpecification: true,
      extraRequirements:
        "Ưu tiên mốc thời gian, sự kiện tiêu biểu và câu hỏi vận dụng bối cảnh.",
    };
  }
  return null;
}

export function getWorksheetSamplePrefill(
  sampleId: string,
): Partial<WorksheetInput> | null {
  ...
  return {
    ...
    objective:
      "Nhận biết dạng phương trình, giải phương trình, vận dụng vào bài toán thực tế.",
    exerciseCount: 6,
    level: "Vừa" as WorksheetInput["level"],
    includeAnswers: true,
    style: "Dễ hiểu" as WorksheetInput["style"],
    extraRequirements:
      "Sắp xếp bài tập từ cơ bản đến vận dụng, có đáp án để giáo viên rà soát.",
  };
}

export function getGenericSamplePrefill(
  sampleId: string,
  href: string,
): Partial<GenericToolInput> | null {
  if (href === "/tools/lesson-plan" && sampleId === "lesson-literature-9") {
    return {
      ...
      duration: "45 phút",
      objectives:
        "Nhận biết vấn đề nghị luận, lập dàn ý, viết đoạn văn nghị luận.",
      methods: "Thảo luận nhóm, luyện tập cá nhân",
      materials: "Sách giáo khoa, bảng phụ, phiếu học tập",
      extraRequirements:
        "Bản nháp cần giáo viên rà soát trước khi sử dụng trên lớp.",
    };
  }
  return null;
}
