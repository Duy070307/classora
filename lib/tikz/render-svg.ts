...
export function renderTikzDraftToSvg(draft: TikzDiagramDraft, options: SemanticSvgSettings = {}) {
  ...
  for (const object of draft.objects) {
    ...
    if (object.type === "arc" || object.type === "angle_marker") { const center = object.position || coords[0]; if (center) { const at = xy(center); const radius = Math.max(10, Number(object.radius || 0.32) * scale); const start = Number(object.metadata?.startAngle ?? 0); const end = Number(object.metadata?.endAngle ?? 90); const polar = (degrees: number) => ({ x: at.x + radius * Math.cos(degrees * Math.PI / 180), y: at.y - radius * Math.sin(degrees * Math.PI / 180) }); const from = polar(start); const to = polar(end); const largeArc = Math.abs(end - start) > 180 ? 1 : 0; parts.push(`<path d="M ${from.x} ${from.y} A ${radius} ${radius} 0 ${largeArc} 0 ${to.x} ${to.y}" stroke-width="${Math.max(1.2, strokeWidth - 0.4)}"/>`); } }
