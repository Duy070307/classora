...
export function DashboardOnboarding() {
  ...
  return <section className="motion-enter relative mb-5 border-y border-blue-100 bg-blue-50/50 px-3 py-3"><button type="button" onClick={dismiss} className="ui-icon-button absolute right-1 top-1" aria-label="Ẩn hướng dẫn"><X size={18} /></button><div className="pr-11 sm:flex sm:items-start sm:justify-between sm:gap-5"><div><p className="text-sm font-semibold text-slate-900">Bắt đầu với Soạn Lab trong 3 phút</p><p className="mt-1 text-sm leading-6 text-slate-600">{steps.map((step, index) => <span key={step} className="mr-3 inline-flex items-center gap-1"><CheckCircle2 size={14} className="text-blue-700" /><span className="font-semibold text-blue-800">{index + 1}.</span>{step}</span>)}</p></div><Link href="/teacher-testing-guide" className="mt-2 inline-flex min-h-11 shrink-0 items-center text-sm font-semibold text-blue-700 hover:text-blue-800 hover:underline sm:mt-0">Xem hướng dẫn</Link></div></section>;
}
