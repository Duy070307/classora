...
export function LessonSlidesWorkspace() {
  ...
  return <div className="space-y-4">
    <ToolPageHeader title="Tạo slide bài giảng" category="Công cụ bài giảng" accent="violet" exportable={false} description="Tạo dàn ý, rà soát từng slide, chỉnh sửa nội dung và xuất PowerPoint có thể tiếp tục biên tập." />
    <p className="border-l-2 border-amber-400 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950">Nội dung là bản nháp hỗ trợ giáo viên và cần được kiểm tra trước khi trình chiếu.</p>
    <WorkflowStageNavigation activeId={stage} onChange={(value) => setStage(value as Stage)} items={[{ id: "source", label: "Chọn nội dung" }, { id: "outline", label: "Kiểm tra dàn ý", disabled: !deck }, { id: "editor", label: "Chỉnh sửa & xuất", disabled: !deck }]} />
...
function SourceStep({ mode, setMode, settings, updateSettings, source, setSource, documents, useDocument, upload, busy, makeOutline }: { mode: SlideSourceType; setMode: (value: SlideSourceType) => void; settings: SlideGenerationSettings; updateSettings: <K extends keyof SlideGenerationSettings>(key: K, value: SlideGenerationSettings[K]) => void; source: SlideSource; setSource: React.Dispatch<React.SetStateAction<SlideSource>>; documents: GeneratedDocument[]; useDocument: (document: GeneratedDocument) => void; upload: (file?: File) => Promise<void>; busy: boolean; makeOutline: () => void }) {
  ...
  return <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]"><section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><SourceModeTabs value={mode} onChange={(value) => setMode(value as SlideSourceType)} items={sourceModes.map((item) => ({ id: item.value, label: item.label }))} />
