  ...
  return (
    <div>
      ...
      <div className="mt-2 flex items-center gap-2 text-[11px] font-extrabold uppercase tracking-wide text-slate-500">
        ...
        {account.role === "admin" ? "Admin" : "Giáo viên"}
      </div>
      <div className="mt-3 flex flex-wrap gap-3 text-xs font-semibold">
        {account.role === "admin" ? <Link href="/admin" className="text-blue-700">Admin</Link> : null}
        <Link href="/settings" className="text-slate-500">Cài đặt</Link>
        <form action="/api/auth/logout" method="post">
          <button className="inline-flex items-center gap-1 text-slate-500" type="submit"><LogOut size={13} />Thoát</button>
        </form>
      </div>
    </div>
  );
