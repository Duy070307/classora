    ...
    return (
      <div className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
        ...
        <div className="mt-3 flex gap-4 text-xs font-semibold">
          <Link href="/settings" className="inline-flex min-h-11 items-center text-slate-600 hover:text-blue-700">Cài đặt</Link>
          <Link href="/data" className="inline-flex min-h-11 items-center text-slate-600 hover:text-blue-700">Dữ liệu</Link>
        </div>
      </div>
    );
  }
  ...
  if (!account) {
    return (
      <div className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
        ...
        <div className="mt-3 flex gap-4 text-xs font-semibold">
          <Link href="/login" className="inline-flex min-h-11 items-center text-blue-700">Đăng nhập</Link>
          <Link href="/data" className="inline-flex min-h-11 items-center text-slate-600 hover:text-blue-700">Dữ liệu</Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-2xl bg-white p-3 ring-1 ring-slate-200">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-x-3 text-xs font-semibold">
        {account.role === "admin" ? <Link href="/admin" className="inline-flex min-h-11 items-center text-blue-700">Quản trị</Link> : null}
        {account.role === "admin" ? <Link href="/admin/feedback" className="inline-flex min-h-11 items-center text-blue-700">Góp ý</Link> : null}
        <Link href="/settings" className="inline-flex min-h-11 items-center text-slate-600 hover:text-blue-700">Cài đặt</Link>
        <form action="/api/auth/logout" method="post">
          <button className="inline-flex min-h-11 items-center gap-1 text-slate-600 hover:text-blue-700" type="submit">
