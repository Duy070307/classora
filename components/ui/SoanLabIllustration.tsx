...
export function SoanLabIllustration({ variant = "workspace", className = "" }: { variant?: "workspace" | "document" | "empty" | "export"; className?: string }) {
  ...
  return (
    <div className={`relative mx-auto aspect-[4/3] w-full max-w-sm overflow-hidden rounded-xl border border-blue-100 bg-blue-50/60 p-4 shadow-sm ${className}`}>
      <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-blue-200/35 blur-2xl" />
      <div className="absolute -bottom-10 -left-8 h-28 w-28 rounded-full bg-slate-200/45 blur-2xl" />
      <div className="relative flex h-full gap-3">
        ...
        <div className="min-w-0 flex-1 rounded-3xl bg-white p-3 shadow-sm ring-1 ring-blue-100/80">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-md bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-700">{variant === "export" ? "DOCX" : "Soạn Lab"}</span>
          </div>
          ...
          <div className="mt-5 grid grid-cols-2 gap-2">
            ...
            <span className="rounded-lg bg-blue-50 p-2 text-[10px] font-bold text-blue-700"><Download size={12} className="mr-1 inline" />Xuất Word</span>
          </div>
        </div>
      </div>
      <span className="absolute right-4 top-5 rounded-md bg-white px-3 py-1.5 text-[11px] font-bold text-blue-700 ring-1 ring-blue-100"><Sparkles size={12} className="mr-1 inline" />Tạo bản nháp</span>
    </div>
  );
}
