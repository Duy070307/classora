    ...
    <>
      ...
      {open ? <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Lưu vào Ngân hàng TikZ">
        <div className="ui-dialog mx-auto my-10 max-w-lg sm:my-16">
          <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-black text-slate-950">Lưu vào Ngân hàng TikZ</h2><p className="mt-1 text-sm text-slate-600">Mã được lưu vào ngân hàng cá nhân và cần rà soát trước khi dùng.</p></div><button type="button" onClick={() => setOpen(false)} className="ui-icon-button" aria-label="Đóng"><X size={19} /></button></div>
