...
import { BookOpenCheck, CircleHelp, ClipboardList, Database, FileClock, History, Home, MessageCircle, Settings, Tags, Wrench } from "lucide-react";
...
export function Sidebar() {
  return <aside className="border-b border-line bg-white md:sticky md:top-0 md:h-screen md:w-72 md:shrink-0 md:border-b-0 md:border-r">
    <div className="flex flex-wrap items-center justify-between gap-3 p-4 md:block md:p-5"><div><Link href="/" className="text-2xl font-bold text-ink">Classora</Link><p className="mt-1 hidden text-sm text-muted sm:block">Bộ công cụ cho giáo viên Việt Nam</p></div><div className="md:mt-3"><UsageBadge compact /></div></div>
    <nav className="flex gap-2 overflow-x-auto border-t border-line px-4 py-3 md:hidden">
      <Link href="/dashboard" className="btn-secondary shrink-0">Dashboard</Link><Link href="/tools" className="btn-secondary shrink-0">Công cụ</Link><Link href="/history" className="btn-secondary shrink-0">Lịch sử</Link><Link href="/getting-started" className="btn-secondary shrink-0">Bắt đầu</Link>
    </nav>
    <nav className="hidden max-h-[calc(100vh-150px)] overflow-y-auto px-4 pb-5 md:block">
