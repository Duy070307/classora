...
import { blueprintToDocument } from "@/lib/exam-blueprint/workflow";
...
export function FileExamGenerator() {
  ...
  useEffect(() => {
    queueMicrotask(() => {
      ...
      if (!restored) return;
      try { const restoredBlueprint = JSON.parse(restored) as ExamBlueprint; setBlueprint(restoredBlueprint); if (restoredBlueprint.bankMode) setBankMode(restoredBlueprint.bankMode); setMessage("Đã khôi phục cấu trúc. Thầy cô vui lòng kiểm tra trước khi tạo đề."); } catch { /* Bỏ qua bản cũ không hợp lệ. */ }
    ...
    <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <section className="tool-form-card min-w-0">
        {!blueprint ? <>
          ...
          <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-blue-950"><strong>Đề chụp hoặc PDF quét?</strong> Hãy dùng <Link href="/tools/document-recognition" className="font-semibold text-blue-700 underline">Đọc đề từ ảnh/PDF</Link> để nhận dạng và rà soát từng trang trước.</div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-blue-700">Kiểm tra cấu trúc đã đọc</p><h2 className="mt-1 text-xl font-black text-slate-950">{blueprint.sourceName || "Cấu trúc đề"}</h2></div><div className="flex items-center gap-2"><button type="button" className="btn-secondary" onClick={()=>openExamBlueprint({mode:"edit",blueprintDocument:blueprintToDocument(blueprint)})}>Chỉnh sửa trong công cụ ma trận</button><span className={`rounded-full px-3 py-1.5 text-xs font-black ${blueprint.confidence.overall >= 0.75 ? "bg-emerald-100 text-emerald-700" : blueprint.confidence.overall >= 0.5 ? "bg-amber-100 text-amber-800" : "bg-red-100 text-red-700"}`}>Độ tin cậy: {confidenceLabel(blueprint.confidence.overall)}</span></div></div>
