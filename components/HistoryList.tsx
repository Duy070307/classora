...
const creationLabels = {
  matrix: "Tạo từ ma trận",
  specification: "Tạo từ bảng đặc tả",
  previous_exam: "Tạo từ đề cũ",
  lesson_material: "Tạo từ tài liệu",
} as const;
...
export function HistoryList() {
  ...
  function deleteSelected() {
    if (
      !selected.length ||
      !window.confirm(`Xóa ${selected.length} tài liệu đã chọn?`)
    )
      return;
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchType =
        typeFilter === "Tất cả" || labels[item.type] === typeFilter;
      const matchFolder =
        folderFilter === "Tất cả thư mục" ||
        (item.folder || "Khác") === folderFilter;
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="ui-panel p-3 sm:p-4">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_200px_200px_auto]">
          ...
          >
            ...
            {Array.from(new Set(Object.values(labels))).map((label) => (
      ...
      {selected.length ? <section className="sticky top-[68px] z-10 rounded-xl border border-blue-200 bg-white/95 p-3 shadow-sm backdrop-blur">
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex min-h-11 items-center gap-2 rounded-lg bg-blue-50 px-3 py-2 text-sm font-semibold text-slate-800">
            <input
              type="checkbox"
          ...
          <select
            ...
            onChange={(event) => {
              if (event.target.value)
                moveSelected(event.target.value as DocumentFolder);
              event.target.value = "";
            }}
          >
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {filteredItems.length ? (
          filteredItems.map((item) => (
            <article
              key={item.id}
              className={`min-w-0 rounded-xl border bg-white p-4 transition hover:border-blue-300 hover:shadow-sm ${selected.includes(item.id) ? "border-blue-400 bg-blue-50/50 ring-2 ring-blue-100" : "border-slate-200"}`}
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {item.type === "exam" ? (
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-black ${item.auditMeta?.auditStatus === "ready" ? "bg-emerald-100 text-emerald-700" : item.auditMeta?.auditStatus === "needs_fix" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-800"}`}
                      >
                    ...
                    {item.structuredExam ? (
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-black ${item.examSolutionSet?.verificationStatus === "verified" ? "bg-emerald-100 text-emerald-700" : item.examSolutionSet?.verificationStatus === "has_errors" || item.examSolutionSet?.verificationStatus === "needs_review" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-600"}`}
                      >
                        {item.examSolutionSet?.verificationStatus === "verified"
                          ? "Đã xác minh"
                          : item.examSolutionSet?.verificationStatus ===
                                "has_errors" ||
                              item.examSolutionSet?.verificationStatus ===
                                "needs_review"
                            ? "Cần rà soát đáp án"
                            : item.examSolutionSet
                              ? "Đã tạo đáp án"
                              : "Chưa có lời giải"}
                      </span>
                    ) : null}
                    {item.generationMeta?.creationMode &&
                    item.generationMeta.creationMode !== "manual" ? (
                      <span className="rounded-md bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">
                        {creationLabels[item.generationMeta.creationMode]}
                      </span>
                    ) : null}
                  </div>
                  <h3 className="mt-3 line-clamp-2 font-black text-slate-900">
                    {item.title}
                  </h3>
                ...
                <button
                  ...
                  className="ui-icon-button shrink-0 border border-slate-200 text-slate-500 hover:border-red-200 hover:bg-red-50 hover:text-red-700"
                  aria-label="Xóa"
                >
                  <Trash2 size={16} />
                </button>
              ...
              <select
                ...
                onChange={(event) => {
                  updateDocumentFolder(
                    item.id,
                    event.target.value as DocumentFolder,
                  );
                  void updateCloudDocumentFolder(
                    item.id,
                    event.target.value as DocumentFolder,
                  );
                  void refresh();
